...
import { VN_PROVINCES, provinceAcceptedNames } from "@/app/lib/vietnam-data";
...
const configs: Record<Mode, Config> = {
  orders: {
    ...
    statusLabels: { draft: "Phiếu tạm", delivering: "Đang giao", paid: "Hoàn thành", cancelled: "Đã hủy", refunded: "Đã trả hàng" },
  },
  invoices: {
    title: "Hóa đơn",
...
const paymentLabel = (value?: string | null) => value === "cash" ? "Tiền mặt" : value === "debt" ? "Công nợ" : value === "transfer" ? "Chuyển khoản" : "---";
...
function DetailPanel({ row, mode, statusLabel }: { row: GridRow; mode: Mode; statusLabel: string }) {
  ...
  async function transitionOrder(status: "delivering" | "paid" | "cancelled", paymentMethod?: "cash" | "debt" | "transfer") {
    if (!order) return;
    setTransitioning(true); setTransitionError("");
    try {
      const response = await fetch("/api/orders", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: order.id, status, payment_method: paymentMethod || null }) });
      const result = await response.json();
  ...
  return <div className="order-detail-panel">
    ...
    {order?.status === "draft" ? <div className="shipment-actions"><button type="button" disabled={transitioning} onClick={() => transitionOrder("delivering")}>Giao hàng</button><button type="button" disabled={transitioning} onClick={() => transitionOrder("paid", "cash")}>Hoàn thành</button><button type="button" disabled={transitioning} onClick={() => transitionOrder("cancelled")}>Hủy phiếu</button></div> : null}
    {order?.status === "delivering" ? <div className="shipment-actions"><button type="button" disabled={transitioning} onClick={() => transitionOrder("paid", "cash")}>Đã thanh toán TM</button><button type="button" disabled={transitioning} onClick={() => transitionOrder("paid", "debt")}>Công nợ</button><button type="button" disabled={transitioning} onClick={() => transitionOrder("cancelled")}>Hủy đơn</button></div> : null}
...
export default function OrderListClient({
  ...
  async function saveOrder(status: "draft" | "delivering" | "paid") {
    ...
    try {
      ...
      setNotice(status === "paid" ? "Đã hoàn thành hóa đơn và cập nhật tồn kho." : status === "delivering" ? "Đã tạo đơn đang giao và xuất kho." : "Đã lưu phiếu đặt hàng.");
    } catch {
...
function InvoiceDetailModal({ order, code, onClose }: { order: SourceOrder; code: string; onClose: () => void }) {
  ...
  const statusLabel = order.status === "paid" ? "Hoàn thành" : order.status === "delivering" ? "Đang giao" : order.status === "refunded" ? "Đã trả hàng" : order.status;
  return <div className="modal-backdrop order-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }} onKeyDown={(event) => { if (event.key === "Escape") onClose(); }}>
    <section role="dialog" aria-modal="true" aria-label="Thông tin hóa đơn" style={{ width: "min(780px,96vw)", maxHeight: "92vh", overflow: "auto", borderRadius: 14, background: "#fff", boxShadow: "0 20px 55px rgba(0,0,0,.25)" }}>
      ...
      <div style={{ padding: "14px 22px 4px", display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        ...
        {order.status === "paid" && <button type="button" onClick={() => { window.location.href = `/sales?edit=${code}`; }} style={{ display: "inline-flex", alignItems: "center", gap: 6, height: 34, padding: "0 14px", border: "1px solid #cfd6de", borderRadius: 7, background: "#fff", color: "#2e3541", cursor: "pointer", fontSize: 13 }}>Chỉnh sửa</button>}
      ...
      <div style={{ padding: "6px 22px 8px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px 28px" }}>
        ...
        <div>
          ...
          {invInfo("Thanh toán", order.status === "paid" ? (order.payment_method === "debt" ? "Công nợ" : order.payment_method === "transfer" ? "Chuyển khoản" : "Đã thanh toán") : order.status === "delivering" ? "Đang giao" : order.status === "refunded" ? "Đã trả hàng" : "---")}
