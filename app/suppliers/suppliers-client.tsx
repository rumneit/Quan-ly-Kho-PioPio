      ...
      <div className="supplier-modal-body">
        <section className="supplier-section"><h3>Thông tin nhà cung cấp</h3>
          <div className="supplier-grid">
            ...
            <label className="supplier-field"><span>Khu vực</span><select value={form.area} onChange={(event) => { const area = event.target.value; setForm((current) => ({ ...current, area, ward: "" })); }}><option value="">Chọn Tỉnh/Thành phố</option>{!(VN_PROVINCES as readonly string[]).includes(form.area) && form.area && <option value={form.area}>{form.area} (tên cũ)</option>}{VN_PROVINCES.map((p) => <option key={p} value={p}>{p}</option>)}</select></label>
            <label className="supplier-field"><span>Phường/Xã</span><input list="supplier-wards" value={form.ward} onChange={(event) => setForm((current) => ({ ...current, ward: event.target.value }))} placeholder={form.area ? "Chọn hoặc nhập Phường/Xã" : "Chọn Tỉnh/TP trước"} disabled={!form.area} />{form.area && <datalist id="supplier-wards">{getWardsForProvince(form.area).map((w) => <option key={w} value={w} />)}</datalist>}</label>
          </div>
        </section>
