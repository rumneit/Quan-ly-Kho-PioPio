      ...
      <div className="supplier-modal-body">
        <section className="supplier-section"><h3>Thông tin nhà cung cấp</h3>
          <div className="supplier-grid">
            ...
            <label className="supplier-field"><span>Phường/Xã</span><select value={form.ward} onChange={(event) => setForm((current) => ({ ...current, ward: event.target.value }))} disabled={!form.area}><option value="">{form.area ? "Chọn Phường/Xã" : "Chọn Tỉnh/TP trước"}</option>{getWardsForProvince(form.area).map((w) => <option key={w} value={w}>{w}</option>)}{form.ward && !getWardsForProvince(form.area).includes(form.ward) && <option value={form.ward}>{form.ward}</option>}</select></label>
          </div>
        </section>
