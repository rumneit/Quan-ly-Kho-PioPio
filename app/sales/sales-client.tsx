...
import { ArrowLeftRight, ClipboardList, Clock, LayoutDashboard, LayoutGrid, List, Menu, Pencil, Phone, Plus, Printer, RefreshCw, Minus, Search, ShoppingCart, Trash2, Truck, Undo2, Zap } from "lucide-react";
...
export default function SalesClient({ profile, products, customers }: Props) {
  ...
  return <main className="pos-shell">
    <header className="pos-topbar">
      <label className="pos-global-search"><Search size={16} /><input ref={productSearchRef} value={query} onChange={event => setQuery(event.target.value)} placeholder="Tìm hàng hóa (F3)" autoFocus /></label>
      ...
      <div className="pos-tools"><button aria-label="Đơn hàng"><ClipboardList size={16} /></button><button aria-label="Hoàn tác"><Undo2 size={16} /></button><button aria-label="Đồng bộ"><RefreshCw size={16} /></button><button aria-label="In"><Printer size={16} /></button>
    ...
    <div className="pos-workspace">
      <section className="pos-left">
        <div className="pos-catalog-head"><span>{filtered.length} hàng hóa</span><div><button className="active" aria-label="Lưới"><LayoutGrid size={16} /></button><button aria-label="Danh sách"><List size={16} /></button></div></div>
        <div className="pos-catalog-grid">{filtered.map(product => <button key={product.id} className="pos-catalog-item" onClick={() => openQty(product)}><i>{product.name.slice(0,2).toUpperCase()}</i><strong>{product.name}</strong><small>{product.sku} · Tồn {product.stock_quantity}</small><b>{money(Number(product.price))}</b><em><Plus size={14} /></em></button>)}{!filtered.length && <div className="pos-blank"><Search size={40} /><strong>Không tìm thấy hàng hóa</strong><p>Thử tìm theo tên hoặc mã hàng khác.</p></div>}</div>
        <div className="pos-order-summary"><label><Pencil size={14} /> <input placeholder="Ghi chú đơn hàng" value={note} onChange={event => setNote(event.target.value)} /></label><div><p><span>Tổng tiền hàng</span><b>{money(total)}</b></p><p><span>Giảm giá</span><b>0</b></p><p className="strong"><span>Khách cần trả</span><b>{money(total)}</b></p></div></div>
      </section>
    ...
    {qtyTarget && <div className="modal-backdrop" onClick={() => setQtyTarget(null)}><section className="pos-qty-modal" onClick={e => e.stopPropagation()}><h3>{qtyTarget.name}</h3><p className="pos-qty-price">{money(Number(qtyTarget.price))} · Tồn {qtyTarget.stock_quantity}</p><div className="pos-qty-control"><button onClick={() => setQty(Math.max(1, qty - 1))}><Minus size={16} /></button><span>{qty}</span><button onClick={() => setQty(Math.min(qtyTarget.stock_quantity, qty + 1))}><Plus size={16} /></button></div><div className="settings-form-actions"><button onClick={() => setQtyTarget(null)}>Hủy</button><button className="settings-btn-primary" onClick={confirmQty} disabled={qty < 1}>Thêm vào đơn</button></div></section></div>}
