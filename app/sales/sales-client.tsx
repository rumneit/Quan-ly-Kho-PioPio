  ...
  async function doOrder(status: "paid" | "draft") {
    ...
    try {
      ...
      if (isDelivery && orderId) {
        ...
        if (!shipRes.ok) { setError("Đơn hàng đã tạo nhưng không thể tạo vận đơn: " + (shipData.error || shipData.error || "")); return; }
      ...
      setCart({}); setNote(""); setReceiverName(""); setReceiverPhone(""); setAddress(""); setArea("");
  ...
  return <main className="pos-shell">
    ...
    <div className="pos-workspace">
      ...
      <aside className="pos-checkout-panel">
        ...
        {isDelivery && <div className="pos-delivery-fields"><input placeholder="Người nhận *" value={receiverName} onChange={e => setReceiverName(e.target.value)} /><input placeholder="Số điện thoại *" value={receiverPhone} onChange={e => setReceiverPhone(e.target.value)} /><input placeholder="Địa chỉ" value={address} onChange={e => setAddress(e.target.value)} /><input placeholder="Khu vực" value={area} onChange={e => setArea(e.target.value)} /></div>}
