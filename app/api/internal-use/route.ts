...
export async function GET() {
  ...
  const { data, error } = await supabase
    .from("internal_use_vouchers")
    .select("*, profiles(full_name)")
    .order("created_at", { ascending: false })
    .limit(500);
  if (error) return NextResponse.json({ error: "Không thể tải phiếu xuất dùng nội bộ." }, { status: 400 });
...
export async function POST(request: Request) {
  ...
  const purpose = String(body.purpose || "Sử dụng nội bộ").trim();
  ...
  if (!validLines.length) return NextResponse.json({ error: "Vui lòng nhập số lượng xuất." }, { status: 400 });
  ...
  const { data: voucher, error: insertError } = await supabase
    ...
    .insert({
      ...
      code: `XNB${String(Date.now()).slice(-9)}`,
      status,
      purpose,
    ...
    .select("id")
    .single();
  if (insertError || !voucher) return NextResponse.json({ error: "Không thể lưu phiếu xuất dùng nội bộ." }, { status: 400 });

  const { error: linesError } = await supabase.from("internal_use_lines").insert(lineRows.map((line) => ({ voucher_id: voucher.id, ...line })));
  ...
  if (status === "completed") {
    for (const line of lineRows) {
      ...
      await supabase.from("inventory_movements").insert({ store_id: profile.store_id, product_id: line.product_id, type: "internal_use", quantity: -line.quantity, note: `Phiếu xuất dùng nội bộ ${voucher.id}`, created_by: profile.id });
    }
  }
