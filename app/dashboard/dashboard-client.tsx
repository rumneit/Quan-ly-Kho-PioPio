...
export default function DashboardClient({ profile, products: initialProducts, metrics }: Props) {
  ...
  const [chartMetric, setChartMetric] = useState("Doanh thu thuần");
  const [rankingMetric, setRankingMetric] = useState("Theo doanh thu thuần");
    ...
    <main className="kv-main">
      ...
      <div className="kv-dashboard-grid">
        <div className="kv-main-column">
          ...
          <section className="kv-card kv-chart-card kv-enter delay-1 kv-full-width"><div className="kv-card-title"><h2>{chartMetric} <b>{money(metrics.revenue)}</b></h2><div className="kv-chart-filters"><select aria-label="Chỉ tiêu" value={chartMetric} onChange={event => setChartMetric(event.target.value)}><option>Doanh thu thuần</option><option>Theo số lượng</option></select><select aria-label="Khoảng thời gian" value={dateRange} onChange={event => setDateRange(event.target.value)}><option>Hôm nay</option><option>Hôm qua</option><option>7 ngày qua</option><option>Tháng này</option><option>Tháng trước</option></select></div></div><div className="kv-tabs"><button className={chartMode === "day" ? "active" : ""} onClick={() => setChartMode("day")}>Theo ngày</button><button className={chartMode === "hour" ? "active" : ""} onClick={() => setChartMode("hour")}>Theo giờ</button><button className={chartMode === "weekday" ? "active" : ""} onClick={() => setChartMode("weekday")}>Theo thứ</button></div><div className="kv-chart"><div className="kv-y-axis"><span>10tr</span><span>7,5tr</span><span>5tr</span><span>2,5tr</span><span>0</span></div><div className="kv-bars">{chartData.values.map((height,index) => <i key={index} style={{ height: metrics.revenue ? `${height}%` : "2px" }}><span>{chartData.labels[index]}</span></i>)}</div>{!metrics.revenue && <div className="kv-no-chart">Chưa có dữ liệu doanh thu trong {dateRange.toLowerCase()}</div>}</div></section>
          <div className="kv-rank-grid kv-full-width"><section className="kv-card kv-rank-card kv-enter delay-2"><div className="kv-card-title"><h2>Top 10 hàng bán chạy</h2><select aria-label="Tiêu chí xếp hạng" value={rankingMetric} onChange={event => setRankingMetric(event.target.value)}><option>Theo doanh thu thuần</option><option>Theo số lượng</option></select></div>{products.length ? <div className="kv-top-products">{products.slice(0,5).map((product,index) => <div key={product.id}><b>{index + 1}</b><span>{product.name}<small>{product.sku}</small></span><strong>{rankingMetric === "Theo số lượng" ? `${product.stock_quantity} sản phẩm` : money(Number(product.price))}</strong></div>)}</div> : <div className="kv-empty"><span>▤</span><p>Chưa có dữ liệu</p><button onClick={() => setModal("product")}>Thêm hàng hóa</button></div>}</section><section className="kv-card kv-rank-card kv-enter delay-3"><div className="kv-card-title"><h2>Top 10 khách mua nhiều nhất</h2><select aria-label="Khoảng thời gian khách hàng"><option>Tháng này</option><option>Tháng trước</option></select></div><div className="kv-empty"><span>♙</span><p>Chưa có dữ liệu</p></div></section></div>
        </div>
