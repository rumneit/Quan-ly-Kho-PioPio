...
export default function DashboardClient({ profile, products: initialProducts, metrics }: Props) {
  ...
  return <div className="secure-app">
    <aside className="sidebar"><div className="brand"><span className="brand-mark">P</span><div><strong>PioPio</strong><small>Quản lý bán hàng</small></div></div><nav className="main-nav"><p className="nav-label">VẬN HÀNH</p><button className="nav-item active"><span>⌂</span>Tổng quan</button><button className="nav-item"><span>▣</span>Bán hàng</button><button className="nav-item"><span>▤</span>Đơn hàng</button><button className="nav-item"><span>◇</span>Sản phẩm</button><button className="nav-item"><span>▧</span>Kho hàng</button><button className="nav-item"><span>♙</span>Khách hàng</button>{profile.role === "manager" && <><p className="nav-label">QUẢN TRỊ</p><button className="nav-item" onClick={() => setModal("staff")}><span>♧</span>Nhân viên</button><button className="nav-item"><span>↗</span>Báo cáo</button><button className="nav-item"><span>⚙</span>Thiết lập</button></>}</nav></aside>
    <div className="app-shell"><header className="topbar"><div className="topbar-left"><div><h1>Tổng quan</h1><p>Dữ liệu cửa hàng được bảo vệ theo quyền truy cập</p></div></div><div className="top-actions"><span className={`role-badge ${profile.role}`}>{profile.role === "manager" ? "Quản lý" : "Bán hàng"}</span><div className="profile"><span>{profile.full_name.split(" ").map(part => part[0]).slice(-2).join("")}</span><div><strong>{profile.full_name}</strong><small>@{profile.username}</small></div></div><form action="/auth/signout" method="post"><button className="button secondary">Đăng xuất</button></form></div></header>
      <main className="content"><section className="welcome-row"><div><p className="eyebrow">TỔNG QUAN KINH DOANH</p><h2>Xin chào, {profile.full_name}</h2></div><div className="hero-actions">{profile.role === "manager" && <button className="button secondary" onClick={() => setModal("product")}>＋ Thêm sản phẩm</button>}<button className="button primary">▣ Bán hàng</button></div></section>
