...
export default function DashboardClient({ profile, products: initialProducts, metrics }: Props) {
  ...
  return <div className="kv-shell">
    ...
    <nav className="kv-horizontal-nav" aria-label="Menu quản lý"><div className="kv-horizontal-items"><Link className="kv-top-item active" href="/dashboard">Tổng quan</Link>{menuGroups.map(group => <div className="kv-nav-dropdown" key={group.title}><button className="kv-top-item" aria-haspopup="true">{group.title}</button><div className="kv-submenu">{group.sections.map((section,index) => <section key={`${group.title}-${index}`}>{section.label && <h3>{section.label}</h3>}{section.items.map(rawItem => { const [item,badge] = rawItem.split("|"); return <button key={rawItem} onClick={() => { if (item === "Danh sách nhân viên") setModal("staff"); if (item === "Danh sách hàng hóa") window.location.assign("/products"); if (item === "Thiết lập giá") window.location.assign("/pricebook"); if (item === "Kiểm kho") window.location.assign("/stocktakes"); if (item === "Xuất dùng nội bộ") window.location.assign("/internal-use"); if (item === "Xuất hủy") window.location.assign("/damage-items"); if (item === "Nhà cung cấp") window.location.assign("/suppliers"); if (item === "Nhập hàng") window.location.assign("/purchase-orders"); if (item === "Trả hàng nhập") window.location.assign("/purchase-returns"); }}><span>{item}</span>{badge && <em>{badge}</em>}</button>; })}</section>)}</div></div>)}</div><Link className="kv-sale-link" href="/sales"><span>🛒</span>Bán hàng</Link></nav>
