...
export default function DashboardClient({ profile, products: initialProducts, metrics }: Props) {
  ...
  return <div className="kv-shell">
    ...
    <nav className="kv-horizontal-nav" aria-label="Menu quản lý"><div className="kv-horizontal-items"><Link className="kv-top-item active" href="/dashboard">Tổng quan</Link>{menuGroups.map(group => <div className="kv-nav-dropdown" key={group.title}><button className="kv-top-item" aria-haspopup="true">{group.title}</button><div className="kv-submenu">{group.sections.map((section,index) => <section key={`${group.title}-${index}`}>{section.label && <h3>{section.label}</h3>}{section.items.map(rawItem => { const [item,badge] = rawItem.split("|"); return <button key={rawItem} onClick={() => { if (item === "Danh sách nhân viên") setModal("staff"); const paths: Record<string,string> = { "Danh sách hàng hóa":"/products", "Thiết lập giá":"/pricebook", "Kiểm kho":"/stocktakes", "Xuất dùng nội bộ":"/internal-use", "Xuất hủy":"/damage-items", "Nhà cung cấp":"/suppliers", "Nhập hàng":"/purchase-orders", "Trả hàng nhập":"/purchase-returns", "Đặt hàng":"/orders", "Hóa đơn":"/invoices", "Trả hàng":"/returns", "Đối tác giao hàng":"/delivery-partners", "Vận đơn":"/waybills", "Khách hàng":"/customers", "Sổ quỹ":"/cashflow", "Cuối ngày":"/end-of-day-report" }; if (paths[item]) window.location.assign(paths[item]); }}><span>{item}</span>{badge && <em>{badge}</em>}</button>; })}</section>)}</div></div>)}</div><Link className="kv-sale-link" href="/sales"><span>🛒</span>Bán hàng</Link></nav>
