...
export default function ProductFilterSidebar({ filters, categories, suppliers, collapsed, onCollapsedChange, onChange, onCreateCategory }: Props) {
  const [active, setActive] = useState<"category" | "inventory" | "supplier" | "type" | "status" | null>(null);
  ...
  const open = (name: typeof active, element: HTMLButtonElement) => { setActive(name); setAnchor(element); if (name === "category") setDraftCategories(filters.categoryIds); if (name === "supplier") setDraftSuppliers(filters.supplierIds); };
