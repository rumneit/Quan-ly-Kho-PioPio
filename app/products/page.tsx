...
export default async function ProductsPage() {
  ...
  const extended = await supabase.from("products").select("id,name,sku,price,cost,stock_quantity,active,created_at,category_id,supplier_id,product_type,direct_sale,linked_sale_channel,expected_out_of_stock_at,product_categories(name),suppliers(name)").order("created_at", { ascending: false });
